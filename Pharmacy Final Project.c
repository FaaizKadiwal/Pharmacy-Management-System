...
void signIn(){                                                                             
	...
	gets(user.password);
	if(strcmp(user.name, "Faaiz") == 0 && strcmp(user.password, "faaiz123") == 0)                
			manager();	
	else if(strcmp(user.name, "Efa") == 0 && strcmp(user.password, "Efa123") == 0)
			admin();
	else
	{
	FILE *fp=fopen("customers.txt", "r");//r r+ w w+ a a+
...
void signUp(){
	...
	while(fread(&f,sizeof(struct user),1,fp)==1)
...
void mainadminmenu(){
    ...
    gotoxy(9,13);
    printf("%c 6 : Message to Manager  %c",219,219);
	gotoxy(9,14);
    printf("%c 7 : Sign Out            %c",219,219);
	gotoxy(9,15);
	printf("%c  Any other key to exit  %c",219,219);
    gotoxy(9,16);
    printf("%c     Choose an option:   %c",219,219);
...
void message(){
	...
	fclose(fp);
	printf("\n\t\t\t\tYour message has been sent to manager\n");
...
void mainmanagermenu(){
    ...
    gotoxy(9,13);
    printf("%c 6 : Message from Admin  %c",219,219);
    gotoxy(9,14);
	printf("%c 7 : Sign Out            %c",219,219);
	gotoxy(9,15);
    printf("%c  Any other key to exit  %c",219,219);
...
void message_view(){
	...
	for(i=0;i<18;i++)
		printf("%c",219);
	gotoxy(0,5);
	printf("%c ADMIN MESSAGES %c",219,219);
	gotoxy(0,6);
	for(i=0;i<18;i++)
		printf("%c",219);
	FILE *fp;
	fp = fopen("message.txt","a+");
	gotoxy(0,8);
	while(fread(&message, sizeof(message), 1, fp)==1)
    	printf("> %s\n",message);
    fclose(fp);	
}
void customer(){
